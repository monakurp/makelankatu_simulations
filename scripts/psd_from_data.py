...
input_type = 'mass'
nbins = 10  # number of aerosol size bins

folder = '/home/monakurp/makelankatu_simulations/source_data'
os.chdir( ( folder ) )

file_psd_data = 'hietikko2018_fig7.csv'
fileout = 'aerosol_emission_relative_per_bin_{}'.format(nbins)
#--------------------------------------------------------------------------------------------------#
...
if input_type == 'mass':
  dpgv = np.exp( np.log( dpg ) + 3.0 * np.log( sigma )**2 )
  m    = 1.0e-5 # in g/m2/s
  V    = np.array([0.00015*m, 0.008*m, 0.3*m, 0.22*m ])
  V    = V / ( rho * 1e3 ) *1e12 
...
data = np.genfromtxt( file_psd_data, delimiter=',' )
...
np.savez( fileout, bin_limits=bin_limits, relative_per_bin=relative_per_bin )   
