...
'''
Description:
Calculate surface emission rates for aerosol particles and gases in the neighbourhood around
Makelankatu and save in to files PIDS_SALSA and PIDS_CHEM
...
# Filepaths for topography, lanes, street types and orography:
...
# Emission data in g/m/veh calculated in calculate_traffic_emissions.py:
...
QF_per_capita = (a0 + a2*HDD) # anthropogenic heat flux
...
# Aerosol number fraction per size bin: (i.e. size distribution of the emission)
number_fracs = np.zeros([len(ncat), nbins])
number_fracs[0,:] += rpb_hietikko

# Aerosol emission mass fractions per chemical component
...
# Set the missing values to the fill value
...
# gas emissions
ev = pids_chem.createVariable('emission_values', 'f4', ('time','z','y','x','nspecies',),
                              fill_value=-9999.0)
