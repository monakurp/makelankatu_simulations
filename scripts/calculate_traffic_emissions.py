...
import os
import pandas as pd
from mapTools import saveTileAsNumpyZ, numpyArray2Tif, openTifAsNumpy
...
year = 2017
times = [ [ 6,  9,  7], [ 6,  9,  8], [ 6,  9,  9], [ 6,  9, 15], [ 6,  9, 16], [ 6,  9, 20], [ 6,  9, 21],
          [ 6, 14,  7], [ 6, 14,  8], [ 6, 14, 14], [ 6, 14, 15], [ 6, 14, 16], [ 6, 14, 20],
          [12,  5,  7], [12,  5,  8], [12,  5, 12], [12,  5, 13], [12,  5, 16], [12,  5, 17], 
          [12,  7,  7], [12,  7,  8], [12,  7,  9], [12,  7, 11], [12,  7, 12], [12,  7, 13]]
...
f1 = open('EEA_calculated_EF.csv','w+')
...
print >> f1, "# EF in g/m/veh assuming a fleet distribution similar to Makelankatu"
print >> f1, header          
...
for ti in range(len(times)):
  ...
  print( '{}-{} ({}/{})'.format( timestr, timestr2, ti, len(times) ) )
  ...
  for pol in pols:
    ...
    print( pol, EF )
    datarow += ', {:4.3e}'.format( EF / 1000.0 )
    if pol=='PM Exhaust':
      datarow += ', {:4.3e}'.format( BC / 1000.0 )
      datarow += ', {:4.3e}'.format( OC / 1000.0 )
    if pol=='NOx':
      datarow += ', {:4.3e}'.format( NO  / 1000.0 )
      datarow += ', {:4.3e}'.format( NO2 / 1000.0 )
    if pol=='VOC':
      datarow += ', {:4.3e}'.format(0.01 * EF / 1000.0 )
      datarow += ', {:4.3e}'.format(0.4  * EF / 1000.0 )
