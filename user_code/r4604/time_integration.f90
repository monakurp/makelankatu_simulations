...
! Current revisions:
! ------------------
! Add skip_time_do_chemistry
! 
! Former revisions:
! -----------------
! $Id: time_integration.f90 4581 2020-06-29 08:49:58Z suehring $
...
!------------------------------------------------------------------------------!
 SUBROUTINE time_integration
    ...
    USE arrays_3d,                                                                                 &
        ONLY:  diss, diss_p, dzu, e_p, nc_p, ni_p, nr_p, prho, pt, pt_p, pt_init, q, qc_p, qr_p,   &
               q_init, q_p, qi_p, ref_state, rho_ocean, sa_p, s_p, tend, u, u_p, v, vpt, v_p, w_p

#if defined( __parallel )  &&  ! defined( _OPENACC )
    USE arrays_3d,                                                                                 &
        ONLY:  e, nc, ni, nr, qc, qi, qr, s, w
#endif
    ...
    USE bulk_cloud_model_mod,                                                                      &
        ONLY: bulk_cloud_model, calc_liquid_water_content, collision_turbulence,                   &
              microphysics_ice_phase, microphysics_morrison, microphysics_seifert
    ...
    USE chem_modules,                                                                              &
!*******************************************************************************
! Modified by MONA 17 Jul 2020
        ONLY:  bc_cs_t_val, chem_species, communicator_chem, emissions_anthropogenic,              &
               emiss_read_legacy_mode, n_matched_vars, skip_time_do_chemistry
!*******************************************************************************
!         ONLY:  bc_cs_t_val, chem_species, communicator_chem, emissions_anthropogenic,              &
...
!*******************************************************************************
    ...
    USE control_parameters,                                                                        &
        ONLY:  advected_distance_x, advected_distance_y, air_chemistry, average_count_3d,          &
               ...
               random_heatflux, rans_tke_e, run_coupled, salsa,                                    &
    ...
    USE salsa_mod,                                                                                 &
        ONLY: aerosol_number, aerosol_mass, bc_am_t_val, bc_an_t_val, bc_gt_t_val,                 &
              communicator_salsa, nbins_aerosol, ncomponents_mass, ngases_salsa,                   &
...
!*******************************************************************************
! Modified by MONA 17 Jul 2020:
           IF ( air_chemistry  .AND.  time_since_reference_point >= skip_time_do_chemistry )  THEN
              DO  n = 1, nvar
...
!--       Exchange of ghost points (lateral boundary conditions)
          ...
          IF ( humidity )  THEN
             ...
             ENDIF
             IF ( bulk_cloud_model .AND. microphysics_ice_phase )  THEN
...
!*******************************************************************************
! MONA:
       IF ( time_since_reference_point >= skip_time_do_chemistry )  THEN
